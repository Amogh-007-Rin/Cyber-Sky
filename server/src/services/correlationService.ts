...
export class CorrelationService {
     ...
     */
    static async ingestLog(data: LogData) {
        // 1. Save Log
        ...
        if (data.eventType === "PROMPT_INJECTION_ATTEMPT") {
             ...
             await this.createIncident(
                 ...
                 "User attempted to bypass safety guardrails.",
                 "HIGH", 
                 "BLOCK", 
                 [log.id], 
                 "Prompt Injection",
        ...
        if (data.eventType === "HONEYTOKEN_TRIGGERED") {
            await this.createIncident(
                ...
                "The AI model outputted a secret honeytoken, indicating a successful prompt injection and data exfiltration attempt.",
                "CRITICAL",
                "BLOCK",
                [log.id],
                "Data Exfiltration",
                "T1560" // Archive Collected Data (closest fit for exfiltration)
            );
        }
     ...
     */
    static async checkBruteForce(ip: string) {
        ...
        if (failedLogins.length >= THRESHOLD) {
            ...
            await this.createIncident(
                "Brute Force Success",
                `Detected ${failedLogins.length} failed logins followed by a success from IP ${ip}.`,
                "CRITICAL",
                "FLAG",
                logIds,
                "Brute Force Success",
     ...
     */
    static async checkSqlInjection(data: LogData, logId: number) {
        ...
        if (detected) {
            await this.createIncident(
                ...
                `Detected SQL Injection pattern '${detected}' in request from ${data.ip}.`,
                "HIGH",
                "BLOCK",
                [logId],
                "SQL Injection",
     ...
     */
    static async checkDDoS(ip: string) {
        ...
        if (recentLogs >= THRESHOLD) {
            ...
            await this.createIncident(
                ...
                `High traffic volume detected from IP ${ip}: ${recentLogs} requests in ${TIME_WINDOW_SECONDS}s.`,
                "CRITICAL",
                "BLOCK",
                logs.map(l => l.id),
                "DDoS Attack",
    ...
    static async createIncident(
        ...
        try {
            ...
            const aiResponse = await axios.post(`${AI_ENGINE_URL}/summarize`, {
                logs: logs.map(l => ({
